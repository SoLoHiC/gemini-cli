  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Pisces CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Pisces CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Pisces CLI Companion extension successfully installed.',
    );
    ...
    vscode.commands.registerCommand('gemini-cli.runGeminiCLI', async () => {
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Pisces CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Pisces CLI in',
        });
      }

      if (selectedFolder) {
        const geminiCmd = 'pisces';
        const terminal = vscode.window.createTerminal({
          name: `Pisces CLI (${selectedFolder.name})`,
