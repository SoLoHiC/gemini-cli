      ...
      .map((content: Content) => {
        ...
        for (const part of content.parts ?? []) {
          if ('text' in part && part.text) {
            if ('thought' in part && part.thought && role === 'assistant') {
              const thinkingBlock = {
                type: 'thinking',
                thinking: part.text,
              ...
              contentBlocks.push(
                thinkingBlock as unknown as Anthropic.Messages.ContentBlockParam,
              );
            } else {
    ...
    switch (chunk.type) {
      case 'content_block_delta':
        if (chunk.delta.type === 'text_delta') {
          ...
          parts.push({ text: chunk.delta.thinking, thought: true });
