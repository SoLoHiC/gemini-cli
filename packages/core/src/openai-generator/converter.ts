...
interface ExtendedCompletionMessage
  extends OpenAI.Chat.ChatCompletionMessage {
  reasoning_content?: string | null;
  reasoning?: string | null;
}

interface ExtendedCompletionChunkDelta
  extends OpenAI.Chat.ChatCompletionChunk.Choice.Delta {
  reasoning_content?: string | null;
  reasoning?: string | null;
}

function _safeJsonParse<T>(jsonString: string, fallback: T): T {
  try {
...
export class OpenAIContentConverter {
      ...
      if (actualTool.functionDeclarations) {
        for (const func of actualTool.functionDeclarations) {
          if (func.name && func.description) {
            ...
            if (!finalParameters) {
              // Default empty schema for functions without parameters
              finalParameters = { type: 'object', properties: {} };
            } else if (typeof finalParameters === 'object') {
              // Ensure it has a type field
              if (!('type' in finalParameters)) {
                finalParameters = { type: 'object', ...finalParameters };
              }
            }
  ...
  private processContent(
    ...
    if (content.role === 'model' && parsedParts.functionCalls.length > 0) {
      const toolCalls = parsedParts.functionCalls
        .filter((fc) => fc.name && fc.name.trim()) // Filter out function calls without names
        .map((fc, index) => ({
      ...
      // Only add tool_calls if we have valid function calls
      if (toolCalls.length > 0) {
        ...
        if (parsedParts.thoughtParts.length > 0) {
          assistantMessage['reasoning_content'] =
            parsedParts.thoughtParts.join('');
        }
        messages.push(assistantMessage);
        return;
      }
      // If no valid tool calls, fall through to regular message handling
    }
  ...
  private createMultimodalMessage(
    role: 'user' | 'assistant',
    parsedParts: Pick<ParsedParts, 'textParts' | 'thoughtParts' | 'mediaParts'>,
  ): OpenAI.Chat.ChatCompletionMessageParam | null {
    const { textParts, thoughtParts, mediaParts } = parsedParts;
    const content = textParts.map((text) => ({ type: 'text' as const, text }));

    // If no media parts, return simple text message
    if (mediaParts.length === 0) {
      // For assistant messages, we must always return a valid message
      // even if there's no content, to satisfy OpenAI API requirements
      if (role === 'assistant') {
        const assistantMessage = {
          role: 'assistant' as const,
        ...
        if (thoughtParts.length > 0) {
          assistantMessage['reasoning_content'] = thoughtParts.join('');
        }
  ...
  convertOpenAIResponseToGemini(
    ...
    const parts: Part[] = [];
    const extendedMessage = choice.message as ExtendedCompletionMessage;
    const reasoningText =
      extendedMessage.reasoning_content ?? extendedMessage.reasoning;
      ...
      for (const toolCall of choice.message.tool_calls) {
        if (toolCall.type === 'function') {
          ...
          if (toolCall.function.arguments) {
            args = _safeJsonParse(toolCall.function.arguments, {});
          }
  ...
  convertOpenAIChunkToGemini(
    ...
    if (choice) {
      const parts: Part[] = [];
      const extendedDelta = choice.delta as ExtendedCompletionChunkDelta;
      const reasoningText =
        extendedDelta.reasoning_content ?? extendedDelta.reasoning;
  ...
  convertGeminiResponseToOpenAI(
    ...
    const choiceMessage = {
      ...
      refusal: null,
    } as OpenAI.Chat.ChatCompletionMessage & Record<string, unknown>;
    if (content?.parts) {
      const thoughtParts = content.parts
        .filter(
        ...
        .map((part) => part.text);
      if (thoughtParts.length > 0) {
        choiceMessage['reasoning_content'] = thoughtParts.join('').trimEnd();
      }
    }
      ...
      if (
        ...
        const validToolCalls = message.tool_calls.filter(
          (toolCall: OpenAI.Chat.ChatCompletionMessageToolCall) =>
            toolCall.id && toolResponseIds.has(toolCall.id),
        );
        ...
        if (validToolCalls.length > 0) {
          ...
          // If no valid tool calls and no content, we still need to keep the message
          // to maintain conversation continuity and satisfy OpenAI API requirements
          const cleanedMessage = { ...message };
          delete (
            cleanedMessage as OpenAI.Chat.ChatCompletionMessageParam & {
              tool_calls?: OpenAI.Chat.ChatCompletionMessageToolCall[];
          ...
          // Ensure the message has at least an empty string for content
          (
            cleanedMessage as OpenAI.Chat.ChatCompletionMessageParam & {
              content: string | null;
            }
    ...
    for (const message of messages) {
      if (message.role === 'assistant' && merged.length > 0) {
        ...
        if (lastMessage.role === 'assistant') {
          ...
          // Ensure the message has either content or tool_calls to satisfy OpenAI API requirements
          (
          ...
          if (combinedToolCalls.length > 0) {
            ...
            // If there are no tool calls and no content, we need to ensure
            // the message has at least an empty string for content
            (
              lastMessage as OpenAI.Chat.ChatCompletionMessageParam & {
                content: string | null;
                tool_calls?: OpenAI.Chat.ChatCompletionMessageToolCall[];
