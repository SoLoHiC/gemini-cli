...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    ...
    it('should return false for non-DashScope URLs', () => {
      const configs = [
        {
          authType: AuthType.OPENAI_COMPATIBLE,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        'User-Agent': `GeminiCLI/unknown (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `GeminiCLI/unknown (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.OPENAI_COMPATIBLE,
      });
    });
  });
  ...
  describe('output token limits', () => {
    it('should limit max_tokens when it exceeds model limit for qwen3-coder-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        ...
        max_tokens: 2000000, // Exceeds the DEFAULT_TOKEN_LIMIT (1,048,576)
      };

      const result = provider.buildRequest(request, 'test-prompt-id');

      expect(result.max_tokens).toBe(1_048_576); // Should be limited to DEFAULT_TOKEN_LIMIT
    });
    ...
    it('should not modify max_tokens when it is within model limit', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'qwen3-coder-plus',
        messages: [{ role: 'user', content: 'Hello' }],
        max_tokens: 1000, // Within the DEFAULT_TOKEN_LIMIT (1,048,576)
      };
    ...
    it('should preserve other request parameters when limiting max_tokens', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'qwen3-coder-plus',
        messages: [{ role: 'user', content: 'Hello' }],
        max_tokens: 100000, // Within DEFAULT_TOKEN_LIMIT (1,048,576)
        temperature: 0.8,
        top_p: 0.9,
        frequency_penalty: 0.1,
        presence_penalty: 0.2,
      ...
      // max_tokens should not be limited (within DEFAULT_TOKEN_LIMIT)
    ...
    it('should work with vision models and output token limits', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        ...
        max_tokens: 20000, // Within DEFAULT_TOKEN_LIMIT (1,048,576)
      };

      const result = provider.buildRequest(request, 'test-prompt-id');

      expect(result.max_tokens).toBe(20000); // Should not be limited (within DEFAULT_TOKEN_LIMIT)
      expect(
    ...
    it('should set high resolution flag for qwen3-vl-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        ...
        max_tokens: 50000, // Within DEFAULT_TOKEN_LIMIT (1,048,576)
      };

      const result = provider.buildRequest(request, 'test-prompt-id');

      expect(result.max_tokens).toBe(50000); // Should not be limited (within DEFAULT_TOKEN_LIMIT)
      expect(
        (result as { vl_high_resolution_images?: boolean })
          .vl_high_resolution_images,
      ).toBe(true);
    ...
    it('should set high resolution flag for the vision-model alias', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        ...
        max_tokens: 9000, // Within DEFAULT_TOKEN_LIMIT (1,048,576)
      };

      const result = provider.buildRequest(request, 'test-prompt-id');

      expect(result.max_tokens).toBe(9000); // Should not be limited (within DEFAULT_TOKEN_LIMIT)
      expect(
        (result as { vl_high_resolution_images?: boolean })
          .vl_high_resolution_images,
      ).toBe(true);
    ...
    it('should handle streaming requests with output token limits', () => {
      ...
      expect(result.max_tokens).toBe(100000); // Should not be limited (within DEFAULT_TOKEN_LIMIT)
